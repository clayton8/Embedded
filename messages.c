...
signed char recv_msg(msg_queue *qptr, unsigned char maxlength, unsigned char *msgtype, void *data) {
    ...
    // Get slot in the qeue
    slot = qptr->cur_read_ind;
    // Get the message slot in the queue
    qmsg = &(qptr->queue[slot]);
    // See if there is any message in that particular queue spot
    if (qmsg->full == 1) {
        ...
        tlength = qmsg->length;
        // Copy qmsg->data into the passed in variable data. Tlength is how many bits it is.
